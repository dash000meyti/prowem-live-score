...
class UpdateMatchResultTest extends TestCase
{
    ...
    public function test_a_finished_match_cannot_return_to_in_play(): void
    {
        ...
        $response
            ...
            ->assertJsonPath(
                'code',
                InvalidGameStatusTransition::CODE
            )
            ->assertJsonPath(
                'message',
    ...
    public function test_repeating_the_same_update_keeps_the_same_persisted_match_state(): void
    {
        $game = Game::query()->firstOrFail();
    ...
    public function test_successful_result_update_dispatches_tournament_updated_event(): void
    {
        Event::fake([
        ...
        Event::assertDispatched(
            TournamentUpdated::class,
            function (TournamentUpdated $event) use ($game): bool {
                return $event->match['id'] === $game->id
                    && $event->match['home_score'] === 2
                    && $event->match['away_score'] === 1
                    && $event->match['status'] === GameStatus::InPlay->value
                    && count($event->standings) === 4;
            }
        );
    }

    public function test_rejected_result_update_does_not_dispatch_tournament_updated_event(): void
    {
        Event::fake([
            TournamentUpdated::class,
        ]);
