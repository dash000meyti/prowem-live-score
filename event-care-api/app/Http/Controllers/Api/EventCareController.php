...
class EventCareController extends Controller
{
    ...
    public function overview(Event $event): JsonResponse
    {
        ...
        $data = ['event' => $eventData, 'readiness' => $readiness, 'readiness_dimensions' => $readiness['dimensions'], 'needs_attention' => ReadinessCheckResource::collection($event->readinessChecks()->whereIn('status', ['blocked', 'warning'])->limit(10)->get())->resolve(request()), 'open_critical_incidents' => IncidentResource::collection($event->incidents()->with('ticket')->where('status', '!=', 'resolved')->where('severity', 'critical')->latest('started_at')->limit(5)->get())->resolve(request()), 'open_tickets' => SupportTicketResource::collection($event->tickets()->with(['assignee', 'event'])->where('status', '!=', 'resolved')->orderBy('sla_due_at')->limit(5)->get())->resolve(request()), 'next_matches' => $event->fixtures()->with('venue')->where('kickoff_at', '>=', now())->orderBy('kickoff_at')->limit(5)->get()->map(fn ($f) => ['id' => $f->id, 'number' => $f->number, 'kickoff_at' => $f->kickoff_at->toISOString(), 'status' => $f->status, 'field' => $f->venue?->name]), 'recent_activity' => ActivityResource::collection($event->activities()->with('actor')->latest('occurred_at')->limit(10)->get())->resolve(request())];
