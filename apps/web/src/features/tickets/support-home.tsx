...
export function SupportHome() {
  ...
  return <div className="support-home">
    <header className="support-home-heading"><div><h1>{supportUser ? "Event Support Queue" : "PROWEM Support"}</h1><p>{supportUser ? "Prioritize customer impact and protect the active SLA." : "See active support, responses and resolved requests."}</p></div>{supportUser ? null : <Link href={`/events/${eventId}/tickets/new`}><Headphones />Request support</Link>}</header>
    {data.critical ? <CriticalSupport ticket={data.critical} eventId={eventId} supportUser={supportUser} /> : <div className="support-clear glass-panel"><Check /><div><h2>No critical support active</h2><p>{supportUser ? "There is no P1 ticket requiring immediate response." : "Your open requests are listed below."}</p></div></div>}
    ...
    {supportUser ? null : <Link id="request-support" className="support-new-request" href={`/events/${eventId}/tickets/new`}><MessageSquare /><span><b>Need help with something else?</b><small>Open a new support request</small></span><ArrowRight /></Link>}
  </div>;
}

function CriticalSupport({ ticket, eventId, supportUser }: { ticket: Ticket; eventId: string; supportUser: boolean }) {
  return <section className="critical-support"><header><span><AlertTriangle />{supportUser ? "P1 requires support action" : "Critical support active"}</span><b>{ticket.priority}</b></header><div className="critical-support-details"><Detail icon={<AlertTriangle />} label="Issue" value={ticket.subject} /><Detail icon={<MapPin />} label="Location" value={ticket.location ?? "Event wide"} /><Detail icon={<UserRound />} label="Owner" value={ticket.assignee ? `${ticket.assignee.name} assigned` : "PROWEM Support assigned"} /><Detail icon={<Clock3 />} label="SLA status" value={remaining(ticket)} danger /></div><div className="critical-support-actions"><Link href={`/events/${eventId}/tickets/${ticket.id}`}><TicketCheck />{supportUser ? "Handle ticket" : "Open ticket"}</Link>{supportUser ? null : <Link href={`/events/${eventId}/tickets/${ticket.id}`} className="secondary"><Headphones />Message support</Link>}</div></section>;
}
